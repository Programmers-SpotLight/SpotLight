...
export async function createSelection(
  ...
  try {
    queryResult = await transaction("selection").insert(
      {
  ...
  if (formData.spots && formData.spots.length > 0) {
    await createSpots(transaction, selectionId, formData.spots);
  }
}
...
export async function saveSelectionImage(imageFile: File): Promise<string> {
  ...
  const fileType: FileTypeResult | undefined = await fileTypeFromBlob(
    imageFile
  );
