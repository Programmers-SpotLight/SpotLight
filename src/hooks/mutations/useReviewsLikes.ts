...
export const useReviewsLikes = (
  ...
  const { mutate: addLikeMutate } = useMutation({
    mutationFn: () => addReviewLike(selectionId, reviewId, spotId),

    onMutate: async () => {
      const previousData = queryClient.getQueryData<IReviewCash>([
        "reviews",
        sltOrSpotId,
        sort
      ]);
    ...
    onError: (error, variables, context) => {
      queryClient.setQueryData(["reviews", sltOrSpotId, sort], {
        ...
        pageParams:
          queryClient.getQueryData<IReviewCash>(["reviews", sltOrSpotId, sort])
            ?.pageParams || []
      });
      ...
      const updatedPages = context?.previousPages.map((page) => ({
        ...
        reviews: page.reviews.map((review) =>
          review.reviewId === reviewId
            ? {
                ...review,
                user: { ...review.user, isLiked: data.liked },
  ...
  const { mutate: removeLikeMutate } = useMutation({
    mutationFn: () => removeReviewLike(selectionId, reviewId, spotId),
      ...
      const updatedPages = previousPages.map((page) => ({
        ...
        reviews: page.reviews.map((review) =>
          review.reviewId === reviewId
            ? {
                ...review,
                user: { ...review.user, isLiked: false },
                likeCount: review.likeCount - 1
              }
            : review
        )
