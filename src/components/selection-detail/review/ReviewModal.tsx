...
const ReviewModal = () => {
  ...
  const handlePictureAdd = (image: string) => {
    setPictures((prevPictures) => [
      ...prevPictures,
      { reviewImageOrder: prevPictures.length, reviewImgSrc: image }
    ]);
  };

  const handlePictureChange = (index: number, newPicture: string) => {
    setPictures((prevPictures) => {
      return prevPictures.map((pic, i) =>
              ...
              <Slider {...settings} className="w-auto flex justify-start">
                ...
                {pictures &&
                  pictures
                    .map((pic, index) => (
                      <div
                        key={pic.reviewImageOrder}
                        className="px-1 inline-block relative"
                      >
                        <PictureInput
                          inputSize="small"
